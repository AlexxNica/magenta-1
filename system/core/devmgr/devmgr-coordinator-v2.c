...
uint32_t log_flags = LOG_ERROR | LOG_INFO | LOG_DEVLC;
...
static mx_status_t dc_start_platform_bus(driver_t* drv, device_t* dev) {
    dev = &platform_device;

    mx_handle_t  hmdi = devmgr_get_mdi();
    if (hmdi == MX_HANDLE_INVALID) {
        return ERR_NOT_SUPPORTED;
    }
    ...
    if (dev->shadow->host == NULL) {
        if ((r = dc_new_devhost("devhost:platform", hmdi, &dev->shadow->host)) < 0) {
            log(ERROR, "devcoord: dh_new_devhost: %d\n", r);
            return r;
        }
        if ((r = dh_create_device(dev->shadow, dev->shadow->host, "")) < 0) {
...
static mx_status_t dc_attempt_bind(driver_t* drv, device_t* dev) {
    ...
    if (is_platform_bus_driver(drv)) {
        return dc_start_platform_bus(drv, dev);
    }
    ...
    // if this device has no devhost, first instantiate it
    if (dev->shadow->host == NULL) {
        if ((r = dc_new_devhost(devhostname, MX_HANDLE_INVALID, &dev->shadow->host)) < 0) {
            log(ERROR, "devcoord: dh_new_devhost: %d\n", r);
            return r;
        }
...
void coordinator(void) {
    ...
    do_publish(&root_device, &platform_device);
